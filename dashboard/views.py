...
def register(request):
    ...
    if User.objects.filter(username=username): return HttpResponse(json.dumps({'is_success':False,'message':f'Username already exists'},indent=4))
...
def collection(request):
    if request.method == 'POST':
        ...
        c=Collection.objects.create(title=d['title'],description=d['description'],user=request.user)
        c.save()
        colUuid=str(c.uuid)
        for m in collectionMovies:
            if Movies.objects.filter(uuid=m['uuid']).exists():
                ...
                movie=Movies.objects.create(title=m['title'],description=m['description'],genres=m['genres'],uuid=m['uuid'])
                movie.save()
                c.movies.add(movie)
        ...
        for c in col:
            cArr.append({'title':c.title,'uuid':str(c.uuid),'description':c.description})
...
def update_collection(request,pk):
    if request.method == 'PUT':
        ...
        try:
            ...
            col.save()
            response.data={'is_success': True,'message':f'Collection {col.title} updated successfully'}
            return response
        ...
        try:
            ...
            for movie in col.movies.all():
                moviesArr.append({'title': movie.title,'description': movie.description,'genres': movie.genres,'uuid': str(movie.uuid)})
        ...
        try:
            col=Collection.objects.get(uuid=pk)
            response.data={'is_success': True,'message':f'Collection {col.title} deleted successfully'}
            col.delete()
            return response
        except Collection.DoesNotExist:
            response.data={'is_success': False,'message':'Collection not found'}
